...
const DissertationTableOptions = ({ record }) => {
    const { errors } = usePage().props;
    const { isEmpty } = useUtils();

    const onDelete = () => {
        Inertia.delete(`/dissertations/${record.id}`);
        if (!isEmpty(errors)) {
    ...
    const onEdit = () => {
        Inertia.get(`/dissertations/${record.id}/edit`);
    };
...
export default DissertationTableOptions;
